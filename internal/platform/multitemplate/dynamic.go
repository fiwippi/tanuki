...
func (tb templateBuilder) buildTemplateFS(f fs.FS) *template.Template {
	switch tb.buildType {
	case templateType:
		return tb.tmpl
	case filesTemplateType:
		return template.Must(template.ParseFS(f, tb.files...))
	case globTemplateType:
		return template.Must(template.ParseGlob(tb.glob))
	case stringTemplateType:
		return template.Must(template.New(tb.templateName).Parse(tb.templateString))
	case stringFuncTemplateType:
		...
		return tmpl
	case filesFuncTemplateType:
		return template.Must(template.New(tb.templateName).Funcs(tb.funcMap).ParseFS(f, tb.files...))
	default:
		panic("Invalid builder type for dynamic template")
	}
}
...
func (r DynamicRender) AddFromFilesFS(name string, f fs.FS, files ...string) *template.Template {
	...
	builder.buildType = filesTemplateType
	r[name] = builder
	return builder.buildTemplateFS(f)
}
